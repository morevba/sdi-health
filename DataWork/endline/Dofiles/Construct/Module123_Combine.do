...
* ******************************************************************** *
*                                                                      *
*              	Merge all 3 module datasets of each country		       *
*				Facility & Provider ID								   *
*                                                                      *
* ******************************************************************** *
* ******************************************************************** *
/*
	  ** PURPOSE: Creates a final merged dataset of all 3 modules across
				  all countries 
...
*******************************************/
		...
		*Open module 3 dataset 
		...
		collapse	diag_accuracy diag1 diag2 diag3 diag4 diag5 diag6 diag7 treat_accuracy	///
					treat1 treat2 treat3 treat4 treat5 treat6 treat7 pph_actions pph_perc 	///
					asphyxia_actions asphyxia_perc score_proc hisex*_st bad_antibio, by(cy facility_id) // irt_score not in this dataset, replace country with cy variable 
		...
		*Create tempfile and save mod 3 dataset 
		sort		cy facility_id 
		tempfile 	mod3
		save 		`mod3', replace
 	
		*Open module 2 dataset 
		use "$EL_dtInt/Module 2/All_countries_mod_2.dta", clear
		...
		*Total staff unavailable but can calculate
		...
		bysort 	country: tab provider_cadre1 is_outpatient, row
		tab 	provider_cadre1 is_outpatient, row
		...
		* replace provider_educ1 = provider_educ2 if 
		...
		bysort country year facility_id: egen mod2 = total(x)
 	
		collapse (mean) num_staff num_med mean_abs mean_unauth_abs mod2 	///
						perc_doc = doctor perc_nurse = nurse 				///
						perc_co = co perc_other = other 					///
						perc_primary = primary perc_secondary = secondary 	///
						perc_postsecondary = postsecondary ///
				 (sum)  fac_doc = doctor fac_co = co fac_nurse = nurse 		///
					    fac_other = other fac_is_out = is_outpatient 		///
						fac_primary = primary fac_secondary = secondary 	///
						fac_postsecond = postsecondary, by(facility_id cy)	// replaced country with cy 
		...
		*Quite a few facilities (38 in Moz, 90 in Nigeria, 43 in Tanzania & more) maxed out at 50 on roster
		tab cy if mod2 == 50 & num_staff > 50
		...
		*Merge all three modules 
		use "$EL_dtInt/Module 1/All_countries_mod_1.dta", clear 
		sort		cy facility_id 
		merge 1:1 	cy facility_id using `mod2', keep(3) nogen 
		merge 1:1 	cy facility_id using `mod3', keep(3) nogen 		
 
		*replace num_med 			= num_med2 if country == "UGANDA" // this is not needed 
		*replace num_staff 			= num_med2 + num_nonmed if country == "UGANDA" // this is not needed 
		...
		foreach var of varlist num_outpatient num_inpatient num_inpatient_beds beds_inpatient num_births {
		...
		*drop num_med2 // this is not needed 
	    ...
	    *Estimate occupancy
		...
		summ 		avg_stay
		replace 	num_inpatient = num_inpatient_beds / `r(mean)' if num_inpatient == 0 & num_inpatient_beds > 0 & num_inpatient_beds != . // Mostly a problem in Uganda
		replace 	num_inpatient_beds = num_inpatient*`r(mean)' if num_inpatient_beds == 0 & num_inpatient > 0 & num_inpatient != . // Mostly a problem in Nigeria
		drop 		avg_stay
		gen 		avg_stay = (num_inpatient_beds/num_inpatient)
		...
		*Labels
		...
		label variable perc_secondary 		"Facility - Providers with secondary education (%)" 
		...
		*Last fixes
		...
		drop if ruralurban == . | facility_level == . | facility_level == .c // replace with ruralurban variable since the variable rural doesnt exist 
 
		*Create facility + rural combo				
		gen 	fac_type = 1 if ruralurban == 1 & facility_level == 1
		replace fac_type = 2 if ruralurban == 1 & facility_level == 2
		replace fac_type = 3 if ruralurban == 1 & facility_level == 3
		replace fac_type = 4 if ruralurban == 2 & facility_level == 1
		replace fac_type = 5 if ruralurban == 2 & facility_level == 2
		replace fac_type = 6 if ruralurban == 2 & facility_level == 3
		...
		gen 	public = publicprivate
		replace public = 0 if publicprivate == 2 | publicprivate == 3 | publicprivate == 4 | publicprivate == 5
		...
		*Create and save tempfile of all 3 modules merged together 
		tempfile mod_123
		save `mod_123', replace
...
*/	
	...
	*Open health expenditure dataset 
	use "$EL_dtInt/dom_gov_health_expend.dta", clear // source = WHO health expenditure database, year 2016
	...
	replace country = "GUINEABISSAU" 	if wbcode == "GNB"
 
	*Create and save tempfile of expenditure dataset 
	tempfile money
	save `money', replace
	...
	*Open all merged module dataset 
	use `mod_123', clear
 	
	*Destring facility id variable 
	...
	*Data source: WHO Global Health Observatory data repository http://apps.who.int/gho/data/node.main.HWFGRP_0020?lang=en
	...
	replace drs_per_10k = 1.274 if country == "GUINEABISSAU"
	replace drs_per_10k = 0.358 if country == "MALAWI"
 
	*Create and save tempfile of this facility level dataset 
	tempfile facildat
	save `facildat', replace // This file also contains unweighted averages of provider knowledge, absenteeism, etc. by facility 
 
	/***********************************************************
		BRING IN MODULE 2 DATA TO GET PROVIDER DEMOGRAPHICS
	*************************************************************/

	*Open module 2 datset 
	use "$EL_dtInt/Module 2/All_countries_mod_2.dta", clear
	
	*Make a variable for whether absenteeism was assessed in this individual
	gen did_abs = 1 if  provider_present2 != .
	
	*Merge mod 2 dataset with module 3 dataset 
	merge 1:1 cy unique_id using "$EL_dtInt/Module 3/All_countries_mod_3.dta"	// 141 observations don't merge from module 3
	...
	*Make an indicator for which people did vignettes 
	gen did_vignette = 1 if _merge == 3
	...
	*Create and save tempfile of this provider level dataste 
	tempfile indiv
	save `indiv', replace 
	...
	*Open the tempfile that contains the facility level dataset 
	use `facildat', clear
	
	*merge on indiv-level data (module 2 + 3)
	merge 1:m cy facility_id using `indiv'
  
	*3,211 (of 77,008) didn't merge - these are module 2 that didn't have a correct facility ID to link to facility data 
	drop if _merge == 2
	drop 	_merge 
...
*	drop  	log_popdesn			// no longer needed 
...
*	label 	variable log_travel "Log of travel time to facility"
	...
	*Save finale harmonized dataset 
