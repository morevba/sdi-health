...
* FOLDER PATHS
	...
	* EDUCATION COUNTRY AND YEAR *
	...
	foreach cy of local cys {
		...
		gl `cy' "${health_data}/`country'/`y'/01_Data/Harmonized/SDI_`country'-`y'_Module3_Harmonized.dta"
	}
...
** COMBINE DATASETS
	...
	append using "${KEN_2018}"
	replace cy = "KEN_2018" if cy == ""
...
* Save dataset for IRT
	* save "Module3_Raw.dta", replace


**History taking**
	** NOTE: DIFFERENT HISTORY TAKING QUESTIONS FOR DIFFERENT COUNTRIES SO CANNOT JUST SUM DIARRHEA_HISTORY_*
	egen nquest1 = anycount(diarrhea_history_symp	diarrhea_history_duration	diarrhea_history_freq	diarrhea_history_consist	diarrhea_history_blood diarrhea_history_vomiting	diarrhea_history_breastfed	diarrhea_history_eating	diarrhea_history_cough	diarrhea_history_fever		diarrhea_history_tears	diarrhea_history_food	diarrhea_history_utensils	diarrhea_history_foodprep		diarrhea_history_handwash	diarrhea_history_othersick	diarrhea_history_deworm	diarrhea_history_med), val(1)
	egen nquest2 = anycount(pneumonia_history_symp	pneumonia_history_coughdur	pneumonia_history_sputum	pneumonia_history_spblood	pneumonia_history_chestpain	pneumonia_history_breath	pneumonia_history_appetit	pneumonia_history_fever	pneumonia_history_gen_cond	pneumonia_history_convulse	pneumonia_history_swallows	pneumonia_history_nose	pneumonia_history_med	pneumonia_history_measles	pneumonia_history_spcolor), val(1)
	egen nquest3 = anycount(diabetes_history_symp	diabetes_history_fever	diabetes_history_convulse	diabetes_history_vomit	diabetes_history_appetite	diabetes_history_thirst	diabetes_history_diarrhea	diabetes_history_cough	diabetes_history_breath	diabetes_history_med	diabetes_history_urine	diabetes_history_numblimb	diabetes_history_smoke	diabetes_history_exercise	diabetes_history_checkups	diabetes_history_tbhiv	diabetes_history_diabetes	diabetes_history_hypertens	diabetes_history_sunkeyes	diabetes_history_dizzy	diabetes_history_headache	diabetes_history_jointpain), val(1)
	egen nquest4 = anycount(tb_history_symp	tb_history_coughdur	tb_history_sputum	tb_history_spblood	tb_history_breath	tb_history_fever	tb_history_night_	tb_history_tb_hh	tb_history_hiv	tb_history_weight	tb_history_appetite	tb_history_gen_cond	tb_history_cough_others	tb_history_repeat	tb_history_med	tb_history_alcohol	tb_history_smoke	tb_history_diet	tb_history_profes	tb_history_sexbeh	tb_history_chestp	tb_history_fevert), val(1)
	egen nquest5 = anycount(malaria_history_feverdur	malaria_history_fevertype	malaria_history_shiver	malaria_history_convulse	malaria_history_vomit	malaria_history_appetite	malaria_history_diarrhea	malaria_history_cough	malaria_history_sevcough	malaria_history_breath	malaria_history_sputum	malaria_history_med	malaria_history_medamt	malaria_history_vacc	malaria_history_sweat), val(1)
	egen nquest6 = anycount(pph_history_symp	pph_history_blood_amt	pph_history_pads	pph_history_parity	pph_history_labor	pph_history_place	pph_history_augme	pph_history_pph	pph_history_fibro	pph_history_amnio	pph_history_anc	pph_history_multi	pph_history_praevia	pph_history_hyper	pph_history_abrup), val(1)
...
**History taking - restricted to most important questions **
	...
	egen nquest2_st = anycount(pneumonia_history_coughdur pneumonia_history_fever pneumonia_history_breath pneumonia_history_measles), val(1)		 /* Duration Fever Difficuly_breathing Measles?	*/	/* NOTE: m3scq3 (able to drink) missing */
	egen nquest3_st = anycount(diabetes_history_appetite diabetes_history_thirst diabetes_history_urine diabetes_history_exercise), val(1)	/* appetite thirst urinary_output exercise	*/		 
	egen nquest4_st = anycount(tb_history_sputum tb_history_spblood tb_history_breath tb_history_nobreath tb_history_fever tb_history_night_sweats tb_history_weightloss tb_history_appetite tb_history_gen_cond), val(1)								/* NOTE: has one additional var - WHAT DOES THIS MEAN?*/
	egen nquest5_st = anycount(malaria_history_feverdur malaria_history_convulse malaria_history_vomit), val(1) /* consciousness was also recommended, not available across countries */
	egen nquest6_st = anycount(pph_history_symp	pph_history_blood_amt	pph_history_pads	pph_history_parity	pph_history_labor	pph_history_place	pph_history_augme	pph_history_pph	pph_history_fibro	pph_history_amnio	pph_history_anc	pph_history_multi	pph_history_praevia	pph_history_hyper	pph_history_abrup), val(1)
...
*Physical examination
	...
	egen nexam1 = anycount(diarrhea_exam_gen_cond diarrhea_exam_temp diarrhea_exam_skin_pinch diarrhea_exam_offer_drink diarrhea_exam_pallor diarrhea_exam_stiff_neck diarrhea_exam_ear_throat diarrhea_exam_rr diarrhea_exam_weightloss diarrhea_exam_weight diarrhea_exam_sunkeyes diarrhea_exam_growthchart diarrhea_exam_feet_swell), val(1)
	egen nexam2 = anycount(pneumonia_exam_rr pneumonia_exam_lowerchest pneumonia_exam_wheezing pneumonia_exam_auscultate pneumonia_exam_nasalflare pneumonia_exam_temp pneumonia_exam_throat pneumonia_exam_ears pneumonia_exam_lymphs), val(1)
	egen nexam3 = anycount(diabetes_exam_temp diabetes_exam_pulse diabetes_exam_abdomen diabetes_exam_weight diabetes_exam_height diabetes_exam_rr diabetes_exam_bp diabetes_exam_oral diabetes_exam_upperextrem diabetes_exam_lowerextrem diabetes_exam_fundoscopy), val(1)
	egen nexam4 = anycount( tb_exam_temp tb_exam_weight tb_exam_height tb_exam_pulse tb_exam_rr tb_exam_auscultate tb_exam_movement tb_exam_bp), val(1)
	egen nexam5 = anycount(malaria_exam_pallor malaria_exam_sunkeyes malaria_exam_paleeyes malaria_exam_gen_cond malaria_exam_skin malaria_exam_temp malaria_exam_pulse malaria_exam_stiff_neck malaria_exam_puffy_face malaria_exam_feet_swell malaria_exam_abdomen malaria_exam_weight malaria_exam_rr), val(1)
	egen nexam6 = anycount(pph_exam_temp pph_exam_pulse pph_exam_weight pph_exam_rr pph_exam_retained pph_exam_bp pph_exam_ruptured pph_exam_tears pph_exam_palpation), val(1)
	egen nexam7 = anycount( asphyxia_exam_hr asphyxia_exam_resp asphyxia_exam_muscle asphyxia_exam_reflex asphyxia_exam_color asphyxia_exam_apgar), val(1)
...
*Physical examination- restricted to most important questions
	egen nexam1_st = anycount(diarrhea_exam_gen_cond diarrhea_exam_skin_pinch diarrhea_exam_offer_drink diarrhea_exam_weight diarrhea_exam_sunkeyes diarrhea_exam_feet_swell), val(1)
	// gen_cond sunk_eyes offer_drink skin_pinch weight oedema
	egen nexam2_st = anycount(pneumonia_exam_rr pneumonia_exam_lowerchest pneumonia_exam_auscultate pneumonia_exam_throat pneumonia_exam_ears), val(1)
...
** DIAGNOSIS ACCURACY	
	...
	// Create diagnoses 
		isid country facility_id provider_id
		...
		replace diag2 = . if skip_pneu == . | skip_pneu == 1
		...
		replace diag3 = . if skip_diab == . | skip_diab == 1 
		...
		replace diag7 = . if skip_asphy == . | skip_asphy == 1 | (asphyxia_diag_neo_asphyxia == . & asphyxia_diag_neo_respdis == . & asphyxia_diag_respdis == .)
		
		// Pregnancy, eclampsia and PID not included for Sierra Leone
...
*****************
	...
	// Diarrhea 
		...
		* replace diarrhea_treat_ors = 1 if diarrhea_treat_ivfluids == 1 & country == "UGANDA"
		gen treat1=(diarrhea_treat_ors == 1 & diarrhea_treat_zinc == 1) | (diarrhea_treat_ivfluids == 1 | diarrhea_treat_ngtube == 1 | diarrhea_treat_tubedose == 1)
	...
	// TB
		* the questionnaire page 16 disaggregates the 4 treatment actions
		// Simple definition 
		gen treat4=(tb_treat_ctdurdose == 1 | tb_treat_ct == 1 | tb_treat_ctdrugs == 1 | tb_treat_ctdur == 1 | tb_treat_ctdose == 1) // slightly different questions in different countries
	...
	// Malaria 
		...
		replace maltreat = . if malaria_treat_al_dose == . & (country == "MADAGASCAR" | country == "MOZAMBIQUE" | country == "NIGER" | country == "SIERRALEONE" | country == "TANZANIA")
	...
	// PPH
		foreach var of varlist pph_treat_oxytocin_wdose pph_treat_oxytocin_iv pph_treat_oxytocin_dose20 pph_treat_oxytocin_ratedrops pph_treat_oxytocin pph_treat_oxytocin_im pph_treat_oxytocine_doseim pph_treat_oxytocin_dose10 pph_treat_oxytocin_rateliter {
			tab country if `var' != .
		...
		egen oxytoc = anycount(pph_treat_oxytocin_wdose pph_treat_oxytocin_iv pph_treat_oxytocin_dose20 pph_treat_oxytocin_ratedrops pph_treat_oxytocin pph_treat_oxytocin_im pph_treat_oxytocine_doseim pph_treat_oxytocin_dose10 pph_treat_oxytocin_rateliter), val(1)
		replace oxytoc = 1 if oxytoc >= 1 & oxytoc != .
		
		egen other_utero_proposed = anycount(pph_treat_prostaglandins pph_treat_misoprostol pph_treat_ergometrine pph_treat_ergometrine_dose2 pph_treat_ergometrine_dose5), val(1)
	...
	// Asphyxia
		...
		* foreach var of varlist aa_* {
			* tab country `var', miss
		* }
		...
		// These are the seven comparable actions across all surveys
		* egen treat7=anycount(aa_sucker aa_babywarm aa_resusmask aa_maskuse aa_heartrate aa_improved aa_call_help aa_30breaths aa_when_stop), val(1)
		* foreach var of varlist aa_sucker aa_babywarm aa_resusmask aa_maskuse aa_heartrate aa_improved aa_call_help aa_30breaths aa_when_stop {
			* replace treat7 = . if `var' == .
		* }
...
// Check relationship between clinical guidelines and diagnosis
	* logit diag1 diarrhea_history_symp	diarrhea_history_duration	diarrhea_history_freq	diarrhea_history_consist	diarrhea_history_blood diarrhea_history_vomiting	diarrhea_history_breastfed	diarrhea_history_eating	diarrhea_history_cough	diarrhea_history_fever		diarrhea_history_tears	diarrhea_history_food	diarrhea_history_utensils	diarrhea_history_foodprep		diarrhea_history_handwash	diarrhea_history_othersick	diarrhea_history_deworm	diarrhea_history_med, or
	* logit diag2 pneumonia_history_symp	pneumonia_history_coughdur	pneumonia_history_sputum	pneumonia_history_spblood	pneumonia_history_chestpain	pneumonia_history_breath	pneumonia_history_appetit	pneumonia_history_fever	pneumonia_history_gen_cond	pneumonia_history_convulse	pneumonia_history_swallows	pneumonia_history_nose	pneumonia_history_med	pneumonia_history_measles	pneumonia_history_spcolor, or
	* logit diag3 diabetes_history_symp	diabetes_history_fever	diabetes_history_convulse	diabetes_history_vomit	diabetes_history_appetite	diabetes_history_thirst	diabetes_history_diarrhea	diabetes_history_cough	diabetes_history_breath	diabetes_history_med	diabetes_history_urine	diabetes_history_numblimb	diabetes_history_smoke	diabetes_history_exercise	diabetes_history_checkups	diabetes_history_tbhiv	diabetes_history_diabetes	diabetes_history_hypertens	diabetes_history_sunkeyes	diabetes_history_dizzy	diabetes_history_headache	diabetes_history_jointpain, or
	* logit diag4 tb_history_symp	tb_history_coughdur	tb_history_sputum	tb_history_spblood	tb_history_breath	tb_history_fever	tb_history_night_	tb_history_tb_hh	tb_history_hiv	tb_history_weight	tb_history_appetite	tb_history_gen_cond	tb_history_cough_others	tb_history_repeat	tb_history_med	tb_history_alcohol	tb_history_smoke	tb_history_diet	tb_history_profes	tb_history_sexbeh	tb_history_chestp	tb_history_fevert, or
	* logit diag5 malaria_history_feverdur	malaria_history_fevertype	malaria_history_shiver	malaria_history_convulse	malaria_history_vomit	malaria_history_appetite	malaria_history_diarrhea	malaria_history_cough	malaria_history_sevcough	malaria_history_breath	malaria_history_sputum	malaria_history_med	malaria_history_medamt	malaria_history_vacc	malaria_history_sweat, or
	* logit diag6 pph_history_symp	pph_history_blood_amt	pph_history_pads	pph_history_parity	pph_history_labor	pph_history_place	pph_history_augme	pph_history_pph	pph_history_fibro	pph_history_amnio	pph_history_anc	pph_history_multi	pph_history_praevia	pph_history_hyper	pph_history_abrup, or
	* logit diag1 diarrhea_exam_gen_cond diarrhea_exam_temp diarrhea_exam_skin_pinch diarrhea_exam_offer_drink diarrhea_exam_pallor diarrhea_exam_stiff_neck diarrhea_exam_ear_throat diarrhea_exam_rr diarrhea_exam_weightloss diarrhea_exam_weight diarrhea_exam_sunkeyes diarrhea_exam_growthchart diarrhea_exam_feet_swell, or
	* logit diag2 pneumonia_exam_rr pneumonia_exam_lowerchest pneumonia_exam_wheezing pneumonia_exam_auscultate pneumonia_exam_nasalflare pneumonia_exam_temp pneumonia_exam_throat pneumonia_exam_ears pneumonia_exam_lymphs, or
	* logit diag3 diabetes_exam_temp diabetes_exam_pulse diabetes_exam_abdomen diabetes_exam_weight diabetes_exam_height diabetes_exam_rr diabetes_exam_bp diabetes_exam_oral diabetes_exam_upperextrem diabetes_exam_lowerextrem diabetes_exam_fundoscopy, or
	* logit diag4 tb_exam_temp tb_exam_weight tb_exam_height tb_exam_pulse tb_exam_rr tb_exam_auscultate tb_exam_movement tb_exam_bp, or
	* logit diag5 malaria_exam_pallor malaria_exam_sunkeyes malaria_exam_paleeyes malaria_exam_gen_cond malaria_exam_skin malaria_exam_temp malaria_exam_pulse malaria_exam_stiff_neck malaria_exam_puffy_face malaria_exam_feet_swell malaria_exam_abdomen malaria_exam_weight malaria_exam_rr, or
	* logit diag6 pph_exam_temp pph_exam_pulse pph_exam_weight pph_exam_rr pph_exam_retained pph_exam_bp pph_exam_ruptured pph_exam_tears pph_exam_palpation, or
...
* SAVE 	
	save "Module3_Prepped.dta", replace
